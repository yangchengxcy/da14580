...
void app_configuration_func(ke_task_id_t const task_id, struct gapm_set_dev_config_cmd *cmd)
{
    ...
    // Device Role
    // cmd->role = GAP_PERIPHERAL_SLV;设置为从模式
	     cmd->role = GAP_CENTRAL_MST; //设置为主模式
    // Device IRK
    ...
    //cmd->con_intv_min = 8;         // 10ms (8*1.25ms) 从模式
    // Slave preferred Maximum of connection interval
    //cmd->con_intv_max = 16;        // 20ms (16*1.25ms) 从模式
    // Slave preferred Connection latency
    //md->con_latency  = 0;        从模式
    // Slave preferred Link supervision timeout
    //cmd->superv_to    = 100;       从模式

...
uint16_t adv_count __attribute__((section("retention_mem_area0")));
uint16_t adv_interval __attribute__((section("retention_mem_area0")));
uint8_t adv_blink_toggle __attribute__((section("retention_mem_area0")));
uint8_t restart_adv;
...
{   
   ...
   if (!sys_startup_flag)
        if (!GPIO_GetPinStatus( GPIO_PORT_1, GPIO_PIN_4 ))
                wkupct_enable_irq(0x1000, 0x0000, 1, 0); // P1_4, polarity high, 1 event, debouncing time = 0
}
...
void app_connection_func(struct gapc_connection_req_ind const *param)
{
    ...
    ke_timer_clear(APP_ADV_TIMER, TASK_APP); //connected -> clear advertise timer
    
    adv_blink_toggle = 0;
    GPIO_SetInactive( GPIO_PORT_1, GPIO_PIN_5); // Tunf off Advertise LED
    ke_timer_clear(APP_ADV_BLINK_TIMER, TASK_APP); //connected -> clear advertise blink timer   
        
    if (param->con_interval > 1600 )// > 400 * 1.25 = 500 ms
        app_set_deep_sleep();
    else
    ...
    if (app_env.conidx != GAP_INVALID_CONIDX)
    {
        ...
        *-------------------------------------------------------------*/
        ...
        #if BLE_BATT_SERVER
        //cur_batt_level = 0;
        app_batt_enable(cur_batt_level, 0, GPIO_BAT_LED_PORT, GPIO_BAT_LED_PIN); //Battery level Alert enabled.
        
        app_batt_poll_start(30000); //5 mins
...
void app_adv_func(struct gapm_start_advertise_cmd *cmd)
{
    ...
    adv_blink_toggle = 0;
    app_timer_set(APP_ADV_BLINK_TIMER, TASK_APP, 10); //100 ms for the first timeout

    if (!adv_count)
       app_timer_set(APP_ADV_TIMER, TASK_APP, 6000); //60s
    
    adv_count++;
    
    if (adv_interval >3200 )// > 2000 ms
        app_set_deep_sleep();
    else
        app_set_extended_sleep();
...
/**
 ****************************************************************************************
 * @brief Advertise timer handler. Checks elapsed advertising time and sets advertise interval accordingly
 *
                                   ...
                                   ke_task_id_t const src_id)
{
    ...
    // then after 1 min goes to 1sec, then after 10 mins goes to 2 sec
    ...
    if( adv_count == 0 )
    {
        ...
        adv_interval = 0x140; // 200ms  (200ms/0.625)
        // Increment counter in retension memory
        adv_count += 1;
		restart_adv = 1;//app_adv_start();
        app_adv_stop();
        app_timer_set(APP_ADV_TIMER, TASK_APP, 6000); //60s
        
    }    
    else if( adv_count > 0  &&  adv_count < 2 ) //Select iterations number to expand advertising with 1ms interval phase.
    {
        if (adv_count == 1 )
        {
            adv_interval = 0x640; // 1 sec  (1600/0.625)ms
            restart_adv = 1;    //set Restart advertising flag. Advertise will restart at advertise stop completion.
            app_adv_stop();
        }
        else // Increment counter in retension memory
            adv_count += 1; 
        
        app_timer_set(APP_ADV_TIMER, TASK_APP, 18000); //180000 ms -> 3 mins
    }
    else // Set device in deep sleep.
    {
        adv_count = 0; 
        adv_interval = 0x140; 
		restart_adv = 0;
        ...
        ke_timer_clear(APP_ADV_BLINK_TIMER, TASK_APP);
...
/**
 ****************************************************************************************
 * @brief Advertise LED blink timer handler. Handles LED blink indicating advertise period. 
 *
 * @param[in] msgid     Id of the message received.
 * @param[in] param     Pointer to the parameters of the message.
 * @param[in] dest_id   ID of the receiving task instance (TASK_GAP).
...
int app_adv_blink_timer_handler(ke_msg_id_t const msgid,
                                 void const *param,
                                 ke_task_id_t const dest_id,
                                 ke_task_id_t const src_id)
{
    ...
    if (adv_blink_toggle)
    {
        ...
        adv_blink_toggle = 0;
        app_timer_set(APP_ADV_BLINK_TIMER, dest_id, 100); //1s OFF
    }
    else
    {
        GPIO_SetActive( GPIO_PORT_1, GPIO_PIN_5);
        adv_blink_toggle = 1;
        app_timer_set(APP_ADV_BLINK_TIMER, dest_id, 5); //50 ms ON
    }
...
/**
 ****************************************************************************************
 * @brief Reinit Advertise LED pin. Called in periph_init().
 *
 * @return void.
 ****************************************************************************************
*/

void app_adv_blink_port_reinit(void)
{

...
void app_proxm_enable(void)
{
    ...
    struct proxm_enable_req * req = KE_MSG_ALLOC(PROXM_ENABLE_REQ, TASK_PROXM, TASK_APP,
                                                 proxm_enable_req);
...
void app_proxm_alert_start(uint8_t lvl)
{
	...
	if (alert_state.lvl == PROXM_ALERT_MILD)
...
void app_proxm_alert_stop(void)
{
	...
	alert_state.lvl = PROXM_ALERT_NONE; //level;
...
{
    app_proxm_init(port, pin);

	if(alert_state.blink_toggle == 1){
        // GPIO_SetActive( alert_state.port, alert_state.pin);
        GPIO_SetActive(XCY_LED0_GPIO);
        GPIO_SetInactive(XCY_LED1_GPIO);
        GPIO_SetActive(XCY_LED2_GPIO);
        GPIO_SetInactive(XCY_LED3_GPIO);
    }
	else{
        // GPIO_SetInactive( alert_state.port, alert_state.pin);
        GPIO_SetInactive(XCY_LED0_GPIO);
        GPIO_SetActive(XCY_LED1_GPIO);
        GPIO_SetInactive(XCY_LED2_GPIO);
        GPIO_SetActive(XCY_LED3_GPIO);
	}

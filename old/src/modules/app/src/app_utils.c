...
printf_msg *printf_msg_list = NULL;
...
{
    printf_msg *node = (printf_msg *)ke_malloc(sizeof(printf_msg), KE_MEM_KE_MSG);
    
    if (!node)
        return NULL;
    
    node->pBuf = (char *)ke_malloc(sz, KE_MEM_KE_MSG);
    if (!node->pBuf) {
        ke_free(node);
        return NULL;
    }
...
static uint8_t sleep_md;

void app_enable_sleep_func(void)
{
    switch(sleep_md) {
       case 0:  break;
...
static void app_disable_sleep_func(void)
{
    sleep_md = app_get_sleep_mode();
    app_disable_sleep();
}
...
int arch_printf(const char *fmt, ...)
{
    ...
    msg = create_msg(len);
    if (msg) {
        bool send2uart = false;
        
        strcpy(msg->pBuf, my_buf);
        // if printf_msg_list is NULL we can send the msg immediately, else 
        ...
        if (!put_to_list(&printf_msg_list, msg))
            ...
            if (send2uart) {
                app_disable_sleep_func();
                uart_write((uint8_t *)msg->pBuf, len, &uart_callback);
            }
        }
    }

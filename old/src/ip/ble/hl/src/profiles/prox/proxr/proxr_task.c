    ...
    if (KE_IDX_GET(src_id) == proxr_env.con_info.conidx)
    {
        ...
        if (char_code != PROXR_ERR_CHAR)
        {
            ...
            if (param->value[0] <= PROXR_ALERT_HIGH)
            {
                ...
                {
                    //Inform APP
					if (char_code == PROXR_IAS_CHAR) //cd  send alert indication only if IAS attribute is writen
            ...
            if ((char_code == PROXR_LLS_CHAR) || param->response)
            {
                // Send Write Response
...
ke_state_t proxr_state[PROXR_IDX_MAX] __attribute__((section("exchange_mem_case1"))); //@WIKRETENTION MEMORY 

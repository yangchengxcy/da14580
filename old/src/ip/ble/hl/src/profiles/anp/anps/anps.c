...
struct anps_env_tag anps_env __attribute__((section("exchange_mem_case1"))); //@WIKRETENTION MEMORY 
/// Alert Notification Profile Server Pool of Environments
struct anps_idx_env_tag **anps_idx_envs __attribute__((section("exchange_mem_case1"))); //@WIKRETENTION MEMORY 
...
void anps_disable(struct anps_idx_env_tag *anps_idx_env, uint16_t conhdl)
{
    ...
    prf_client_disable((prf_env_struct ***)&anps_idx_envs, KE_IDX_GET(anps_idx_env->con_info.prf_id));

...
struct glpc_env_tag **glpc_envs __attribute__((section("exchange_mem_case1"))); //@WIKRETENTION MEMORY 
            ...
            && (racp_req->filter.operator <= GLP_OP_WITHIN_RANGE_OF))
    {
        ...
        else
        {
                    ...
                    || (racp_req->filter.operator == GLP_OP_WITHIN_RANGE_OF))
            {
                ...
                co_write16p(packed_val + cursor,racp_req->filter.val.time.offset_min);
                cursor +=2;
            }
                    ...
                    || (racp_req->filter.operator == GLP_OP_WITHIN_RANGE_OF))
            {
                ...
                co_write16p(packed_val + cursor,racp_req->filter.val.time.offset_max);
                cursor +=2;
            }
        }
    }

                                         ...
                                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    struct gapc_disconnect_cmd *discon_req = KE_MSG_ALLOC(GAPC_DISCONNECT_CMD,
            KE_BUILD_ID(TASK_GAPC, glpc_env->con_info.conidx), dest_id,
            gapc_disconnect_cmd);

    discon_req->operation = GAPC_DISCONNECT;
    discon_req->reason = (uint8_t) CO_ERROR_REMOTE_USER_TERM_CON;
...
ke_state_t glpc_state[GLPC_IDX_MAX] __attribute__((section("exchange_mem_case1"))); //@WIKRETENTION MEMORY 

 ...
 ****************************************************************************************
 */
static uint8_t prf_client_env_free(prf_env_struct ***p_envs, uint8_t conidx);

#endif // #if (BLE_CLIENT_PRF)
...
void prf_cleanup(uint8_t conidx, uint16_t conhdl, uint8_t reason)
{
    ...
    {
        gapc_send_disconect_ind(status, reason, conhdl, prf_task_id);
    }
    ...
    if (ke_state_get(prf_task_id) != NBPC_IDLE)
    {
        gapc_send_disconect_ind(conidx, reason, conhdl, prf_task_id);
    }
    #endif // (BLE_NEB_CLIENT)
    ...
    #if (BLE_NEB_SERVER)
    if ((ke_state_get(TASK_NBPS) == NBPS_CONNECTED) || (ke_state_get(TASK_NBPS) == NBPS_INDICATE))
    {

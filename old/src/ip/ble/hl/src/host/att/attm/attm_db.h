...
/**
 ****************************************************************************************
 * @brief Set the length of the attribute passed as parameter.
 *
 * @param[in]  attm_elmt Attribute element for which length has to be set
 * @param[in]  len    Length of the attribute
 *
 ****************************************************************************************
 */
__INLINE void
attm_set_len(struct attm_elmt* attm_elmt, att_size_t len)
{
    /* sanity check */
    ASSERT_ERR(attm_elmt != NULL);

    /* set the requested permission bits */
    attm_elmt->length = len;
}
